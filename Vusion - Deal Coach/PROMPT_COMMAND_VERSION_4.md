# Deal Coach Prompt Command - VERSION 4 (FINAL - WITH ALL FIXES)
...
**STEP 3: CALCULATE SUBSEQUENT STAGES - CRITICAL INSTRUCTIONS**

**IMPORTANT: Each history record represents a TRANSITION. The NewValue becomes the NEW stage that starts at that CreatedDate.**

- CREATE an empty array called allStages
- ADD the initial stage to allStages FIRST:
  {
...
- FOR EACH record in stageTransitionRecords (loop through EVERY record, index i from 0 to length-1):
  
  **CRITICAL RULES:**
  1. **PROCESS EVERY HISTORY RECORD** - Do NOT skip any records, even if the stage name repeats
  2. **EACH TRANSITION CREATES A NEW STAGE** - Even if "Budget approved" appears twice, create TWO separate stage entries
  3. **The stage name is the NewValue of the CURRENT record** - This is the stage that BEGINS at this transition
  4. **The stage STARTS when this transition occurs** - Use the CreatedDate of the current record as the start date
  
  - Stage Name = NewValue of current record (this is the stage that BEGINS at this transition)
  - PARSE CreatedDate of current record as date
  - Stage Start Date = CreatedDate of current record (this is when the stage BEGINS - use this for display)
  
  - IF this is NOT the last record (i < stageTransitionRecords.length - 1):
    - PARSE CreatedDate of NEXT record (index i+1) as date
    - Stage End Date = CreatedDate of the NEXT record (index i+1) (this is when the NEXT transition occurs)
    - Set isCurrent = false
  - ELSE (this IS the last record):
    - PARSE CurrentDate__c as date
    - Stage End Date = CurrentDate__c (this is the current date, stage is still active)
    - Set isCurrent = true
  
  - CALCULATE Stage Duration = Stage End Date - Stage Start Date (in days, as integer)
  - EXAMPLE: If History 1 CreatedDate = "2019-07-31" (Budget approved starts) and History 2 CreatedDate = "2019-09-12" (Quote Sent starts), then Budget approved duration = Sep 12 - Jul 31 = 43 days
  - EXAMPLE: If History 4 CreatedDate = "2019-11-25" (Budget approved starts again) and History 5 CreatedDate = "2020-03-13" (Closed starts), then second Budget approved duration = Mar 13, 2020 - Nov 25, 2019 = 108 days
  
  - ADD to allStages (DO NOT SKIP, even if stage name already exists):
    {
      name: Stage Name,
  ...
  **VERIFY**: After the loop, you should have EXACTLY (1 initial stage + stageTransitionCount) stages in allStages

**CRITICAL VERIFICATION: After building allStages, verify:**
- Total number of stages in allStages array = 1 (initial) + stageTransitionCount
- totalStagesCount = 1 + stageTransitionCount (must match the count of items in allStages)
- The LAST stage in allStages has isCurrent = true
- The LAST stage name matches {{{StageName}}}
- The LAST stage duration = CurrentDate__c - Last History CreatedDate (NOT 0)
- opportunityAgeDays = CurrentDate__c - CreatedDate (should be a large number, NOT a small number like 192)
- Sum of all durationDays should equal opportunityAgeDays (±1 day tolerance)

**STEP 4: CALCULATE SUMMARY METRICS**
- totalStagesCount = 1 + stageTransitionCount (MUST equal the number of items in allStages array)
- **CRITICAL DATE PARSING**: PARSE CurrentDate__c as date (format: "YYYY-MM-DD HH:MM:SS" or "YYYY-MM-DD")
  - EXAMPLE: "2025-11-17 00:00:00" or "2025-11-17" should parse to Nov 17, 2025
- PARSE CreatedDate of the last record in stageTransitionRecords as date
- **CRITICAL**: currentStageDurationDays = CurrentDate__c - Last History CreatedDate (in days as integer)
  - EXAMPLE: If CurrentDate__c = "2025-11-17" and Last History = "2020-03-13", then currentStageDurationDays = 2095 days (NOT 0)
  - This is the duration of the CURRENT/LAST stage
- PARSE Opportunity CreatedDate as date
- **CRITICAL**: opportunityAgeDays = CurrentDate__c - CreatedDate (in days, as integer)
  - EXAMPLE: If CurrentDate__c = "2025-11-17" and CreatedDate = "2019-07-26", then opportunityAgeDays = 2295 days (NOT 192)
  - This should be a LARGE number for old opportunities (1000+ days), NOT a small number
...
- **VERIFY**: opportunityAgeDays should be a large number (1000+ days for old opportunities), NOT a small number like 192
- **VERIFY**: currentStageDurationDays should NOT be 0 if CurrentDate__c is far in the future from Last History CreatedDate

**CRITICAL: UPDATE LAST STAGE DURATION**
- Find the LAST stage in allStages (where isCurrent = true)
- UPDATE its durationDays to equal currentStageDurationDays (NOT 0)
- This ensures the last stage shows the correct duration from Last History CreatedDate to CurrentDate__c
...
**STEP 6: VERIFY CALCULATIONS - MANDATORY CHECKS**
...
- Verify totalStagesCount = 1 + stageTransitionCount
- Verify you have EXACTLY totalStagesCount stages in allStages array
...
<!-- CRITICAL: startDateFormatted MUST be displayed - it shows when the stage began -->
