GPTfy Skill Package: Get Contact Engagement History
================================

API name : fetch_contact_engagement_history
Category : Contacts
Apex     : ContactAgenticSkillsHandler + AgenticSkillsBase (+ smoke tests)
Prompt   : ccai__AI_Prompt__c Name = fetch_contact_engagement_history
...
2) Deploy Apex (from the unzipped folder):
   ...
   or
   sf project deploy start --manifest package.xml --test-level RunSpecifiedTests --tests AgenticSkillsBaseTest --tests ContactAgenticSkillsHandlerTest --target-org <alias>
...
NOTES
-----
- Apex is shared per object. Deploying ContactAgenticSkillsHandler installs all methods in that class;
  this seed still creates ONLY the "fetch_contact_engagement_history" prompt record.
...
Skill summary
-------------
Timeline of recent tasks, meetings, and emails with the contact.
