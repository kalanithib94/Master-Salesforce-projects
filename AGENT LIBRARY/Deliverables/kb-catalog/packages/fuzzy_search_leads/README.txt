GPTfy Skill Package: Search Leads
================================

API name : fuzzy_search_leads
Category : Leads
Apex     : LeadAgenticSkillsHandler + AgenticSkillsBase (+ smoke tests)
Prompt   : ccai__AI_Prompt__c Name = fuzzy_search_leads
...
2) Deploy Apex (from the unzipped folder):
   ...
   or
   sf project deploy start --manifest package.xml --test-level RunSpecifiedTests --tests AgenticSkillsBaseTest --tests LeadAgenticSkillsHandlerTest --target-org <alias>
...
NOTES
-----
- Apex is shared per object. Deploying LeadAgenticSkillsHandler installs all methods in that class;
  this seed still creates ONLY the "fuzzy_search_leads" prompt record.
...
Skill summary
-------------
Find a prospect by name or company before qualifying or converting.
