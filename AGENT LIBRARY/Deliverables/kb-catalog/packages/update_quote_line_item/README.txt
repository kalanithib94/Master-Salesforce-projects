GPTfy Skill Package: Update Quote Line Item
================================

API name : update_quote_line_item
Category : Quotes
Apex     : QuoteAgenticSkillsHandler + AgenticSkillsBase (+ smoke tests)
Prompt   : ccai__AI_Prompt__c Name = update_quote_line_item
...
2) Deploy Apex (from the unzipped folder):
   ...
   or
   sf project deploy start --manifest package.xml --test-level RunSpecifiedTests --tests AgenticSkillsBaseTest --tests QuoteAgenticSkillsHandlerTest --target-org <alias>
...
NOTES
-----
- Apex is shared per object. Deploying QuoteAgenticSkillsHandler installs all methods in that class;
  this seed still creates ONLY the "update_quote_line_item" prompt record.
...
Skill summary
-------------
Change quantity or price on an existing quote line.
