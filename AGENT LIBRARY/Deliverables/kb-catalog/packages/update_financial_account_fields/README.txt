GPTfy Skill Package: Update Financial Account
================================

API name : update_financial_account_fields
Category : Industry Clouds
Apex     : IndustryAgenticSkillsHandler + AgenticSkillsBase (+ smoke tests)
Prompt   : ccai__AI_Prompt__c Name = update_financial_account_fields
...
2) Deploy Apex (from the unzipped folder):
   ...
   or
   sf project deploy start --manifest package.xml --test-level RunSpecifiedTests --tests AgenticSkillsBaseTest --tests IndustryAgenticSkillsHandlerTest --target-org <alias>
...
NOTES
-----
- Apex is shared per object. Deploying IndustryAgenticSkillsHandler installs all methods in that class;
  this seed still creates ONLY the "update_financial_account_fields" prompt record.
...
Skill summary
-------------
Update financial account status or attributes in FSC orgs.
