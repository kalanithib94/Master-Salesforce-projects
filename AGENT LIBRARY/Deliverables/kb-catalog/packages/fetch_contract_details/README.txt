GPTfy Skill Package: Get Contract Details
================================

API name : fetch_contract_details
Category : Contracts
Apex     : ContractAgenticSkillsHandler + AgenticSkillsBase (+ smoke tests)
Prompt   : ccai__AI_Prompt__c Name = fetch_contract_details
...
2) Deploy Apex (from the unzipped folder):
   ...
   or
   sf project deploy start --manifest package.xml --test-level RunSpecifiedTests --tests AgenticSkillsBaseTest --tests ContractAgenticSkillsHandlerTest --target-org <alias>
...
NOTES
-----
- Apex is shared per object. Deploying ContractAgenticSkillsHandler installs all methods in that class;
  this seed still creates ONLY the "fetch_contract_details" prompt record.
...
Skill summary
-------------
Load contract status, term dates, and account linkage.
