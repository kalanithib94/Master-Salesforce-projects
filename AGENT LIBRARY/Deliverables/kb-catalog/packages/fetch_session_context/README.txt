GPTfy Skill Package: Get Session Context
================================

API name : fetch_session_context
Category : Platform & Insights
Apex     : UtilityAgenticSkillsHandler + AgenticSkillsBase (+ smoke tests)
Prompt   : ccai__AI_Prompt__c Name = fetch_session_context
...
- Apex is shared per object. Deploying UtilityAgenticSkillsHandler installs all methods in that class;
  this seed still creates ONLY the "fetch_session_context" prompt record.
...
Skill summary
-------------
See which record and related people the chat is on, so the agent can act without asking for Ids. Use before updates, summaries, logging activity, or drafting follow-ups.
