GPTfy Skill Package: Get Subscription Details
================================

API name : fetch_subscription_details
Category : Orders & Subscriptions
Apex     : OrderAgenticSkillsHandler + AgenticSkillsBase (+ smoke tests)
Prompt   : ccai__AI_Prompt__c Name = fetch_subscription_details
...
2) Deploy Apex (from the unzipped folder):
   ...
   or
   sf project deploy start --manifest package.xml --test-level RunSpecifiedTests --tests AgenticSkillsBaseTest --tests OrderAgenticSkillsHandlerTest --target-org <alias>
...
NOTES
-----
- Apex is shared per object. Deploying OrderAgenticSkillsHandler installs all methods in that class;
  this seed still creates ONLY the "fetch_subscription_details" prompt record.
...
Skill summary
-------------
Load subscription term, quantity, and billing status.
