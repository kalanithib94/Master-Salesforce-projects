GPTfy Skill Package: Update Opportunity Contact Role
================================

API name : update_opportunity_contact_role
Category : Opportunities
Apex     : OpportunityAgenticSkillsHandler + AgenticSkillsBase (+ smoke tests)
Prompt   : ccai__AI_Prompt__c Name = update_opportunity_contact_role
...
2) Deploy Apex (from the unzipped folder):
   ...
   or
   sf project deploy start --manifest package.xml --test-level RunSpecifiedTests --tests AgenticSkillsBaseTest --tests OpportunityAgenticSkillsHandlerTest --target-org <alias>
...
NOTES
-----
- Apex is shared per object. Deploying OpportunityAgenticSkillsHandler installs all methods in that class;
  this seed still creates ONLY the "update_opportunity_contact_role" prompt record.
...
Skill summary
-------------
Change role or primary flag for someone on the deal.
