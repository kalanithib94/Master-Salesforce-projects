GPTfy Skill Package: Get Resource Availability
================================

API name : fetch_service_resource_availability
Category : Field Service
Apex     : FieldServiceAgenticSkillsHandler + AgenticSkillsBase (+ smoke tests)
Prompt   : ccai__AI_Prompt__c Name = fetch_service_resource_availability
...
2) Deploy Apex (from the unzipped folder):
   ...
   or
   sf project deploy start --manifest package.xml --test-level RunSpecifiedTests --tests AgenticSkillsBaseTest --tests FieldServiceAgenticSkillsHandlerTest --target-org <alias>
...
NOTES
-----
- Apex is shared per object. Deploying FieldServiceAgenticSkillsHandler installs all methods in that class;
  this seed still creates ONLY the "fetch_service_resource_availability" prompt record.
...
Skill summary
-------------
Check when a technician or resource is free to schedule.
