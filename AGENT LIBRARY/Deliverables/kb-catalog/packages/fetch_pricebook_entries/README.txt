GPTfy Skill Package: Get Price Book Entries
================================

API name : fetch_pricebook_entries
Category : Products & Price Books
Apex     : ProductAgenticSkillsHandler + AgenticSkillsBase (+ smoke tests)
Prompt   : ccai__AI_Prompt__c Name = fetch_pricebook_entries
...
2) Deploy Apex (from the unzipped folder):
   ...
   or
   sf project deploy start --manifest package.xml --test-level RunSpecifiedTests --tests AgenticSkillsBaseTest --tests ProductAgenticSkillsHandlerTest --target-org <alias>
...
NOTES
-----
- Apex is shared per object. Deploying ProductAgenticSkillsHandler installs all methods in that class;
  this seed still creates ONLY the "fetch_pricebook_entries" prompt record.
...
Skill summary
-------------
Show list prices for products in a chosen or standard price book.
