GPTfy Skill Package: Get Account Related Records
================================

API name : fetch_account_related_lists
Category : Accounts
Apex     : AccountAgenticSkillsHandler + AgenticSkillsBase (+ smoke tests)
Prompt   : ccai__AI_Prompt__c Name = fetch_account_related_lists
...
2) Deploy Apex (from the unzipped folder):
   ...
   or
   sf project deploy start --manifest package.xml --test-level RunSpecifiedTests --tests AgenticSkillsBaseTest --tests AccountAgenticSkillsHandlerTest --target-org <alias>
...
- Apex is shared per object. Deploying AccountAgenticSkillsHandler installs all methods in that class;
  this seed still creates ONLY the "fetch_account_related_lists" prompt record.
...
Skill summary
-------------
See contacts, opportunities, cases, and other children on the account.
