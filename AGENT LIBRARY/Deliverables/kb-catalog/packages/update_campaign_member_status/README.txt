GPTfy Skill Package: Update Campaign Member Status
================================

API name : update_campaign_member_status
Category : Campaigns
Apex     : CampaignAgenticSkillsHandler + AgenticSkillsBase (+ smoke tests)
Prompt   : ccai__AI_Prompt__c Name = update_campaign_member_status
...
2) Deploy Apex (from the unzipped folder):
   ...
   or
   sf project deploy start --manifest package.xml --test-level RunSpecifiedTests --tests AgenticSkillsBaseTest --tests CampaignAgenticSkillsHandlerTest --target-org <alias>
...
NOTES
-----
- Apex is shared per object. Deploying CampaignAgenticSkillsHandler installs all methods in that class;
  this seed still creates ONLY the "update_campaign_member_status" prompt record.
...
Skill summary
-------------
Mark someone as Sent, Responded, or another member status.
