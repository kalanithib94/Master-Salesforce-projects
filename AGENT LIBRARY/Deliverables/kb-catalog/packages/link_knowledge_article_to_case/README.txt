GPTfy Skill Package: Link Knowledge Article to Case
================================

API name : link_knowledge_article_to_case
Category : Cases
Apex     : CaseAgenticSkillsHandler + AgenticSkillsBase (+ smoke tests)
Prompt   : ccai__AI_Prompt__c Name = link_knowledge_article_to_case
...
2) Deploy Apex (from the unzipped folder):
   ...
   or
   sf project deploy start --manifest package.xml --test-level RunSpecifiedTests --tests AgenticSkillsBaseTest --tests CaseAgenticSkillsHandlerTest --target-org <alias>
...
NOTES
-----
- Apex is shared per object. Deploying CaseAgenticSkillsHandler installs all methods in that class;
  this seed still creates ONLY the "link_knowledge_article_to_case" prompt record.
...
Skill summary
-------------
Attach a helpful article to the case for audit and deflection.
