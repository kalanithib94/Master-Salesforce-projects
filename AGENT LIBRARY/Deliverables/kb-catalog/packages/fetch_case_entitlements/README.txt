GPTfy Skill Package: Get Case Entitlements
================================

API name : fetch_case_entitlements
Category : Service Cloud
Apex     : ServiceAgenticSkillsHandler + AgenticSkillsBase (+ smoke tests)
Prompt   : ccai__AI_Prompt__c Name = fetch_case_entitlements
...
2) Deploy Apex (from the unzipped folder):
   ...
   or
   sf project deploy start --manifest package.xml --test-level RunSpecifiedTests --tests AgenticSkillsBaseTest --tests ServiceAgenticSkillsHandlerTest --target-org <alias>
...
NOTES
-----
- Apex is shared per object. Deploying ServiceAgenticSkillsHandler installs all methods in that class;
  this seed still creates ONLY the "fetch_case_entitlements" prompt record.
...
Skill summary
-------------
See support coverage and entitlement levels for the customer.
