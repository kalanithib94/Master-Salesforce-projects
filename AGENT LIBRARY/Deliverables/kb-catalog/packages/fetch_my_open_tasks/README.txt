GPTfy Skill Package: Get My Open Tasks
================================

API name : fetch_my_open_tasks
Category : Activities & Scheduling
Apex     : ActivityAgenticSkillsHandler + AgenticSkillsBase (+ smoke tests)
Prompt   : ccai__AI_Prompt__c Name = fetch_my_open_tasks
...
2) Deploy Apex (from the unzipped folder):
   ...
   or
   sf project deploy start --manifest package.xml --test-level RunSpecifiedTests --tests AgenticSkillsBaseTest --tests ActivityAgenticSkillsHandlerTest --target-org <alias>
...
NOTES
-----
- Apex is shared per object. Deploying ActivityAgenticSkillsHandler installs all methods in that class;
  this seed still creates ONLY the "fetch_my_open_tasks" prompt record.
...
Skill summary
-------------
Show the current user's incomplete to-dos for the day or week.
