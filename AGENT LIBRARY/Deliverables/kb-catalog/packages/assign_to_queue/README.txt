GPTfy Skill Package: Assign Record to Queue
================================

API name : assign_to_queue
Category : Service Cloud
Apex     : UtilityAgenticSkillsHandler + AgenticSkillsBase (+ smoke tests)
Prompt   : ccai__AI_Prompt__c Name = assign_to_queue
...
- Apex is shared per object. Deploying UtilityAgenticSkillsHandler installs all methods in that class;
  this seed still creates ONLY the "assign_to_queue" prompt record.
...
Skill summary
-------------
Route a record into a work queue for the next available agent.
