...
when 'create_lead'                  { return handleCreateLead(parameters); }
    ...
    /**
     * @description Skill 15 — create_lead. Accepts Lead field API names as flat
    ...
    private String handleCreateLead(Map<String, Object> p) {
        if (!Schema.sObjectType.Lead.isCreateable()) return errorHtml('Could not create lead', 'Lead is not creatable.');
        ...
        for (String required : new List<String>{ 'LastName', 'Company' }) {
            if (!fields.containsKey(required) || String.isBlank(toText(fields.get(required)))) {
                return errorHtml('Could not create lead', required + ' is required.');
            }
        }
        try {
            Lead l = new Lead();
            applyFieldsToSObject(l, fields, false);
            insert l;
            String body = '<ul><li><b>Name:</b> ' + escapeHtml(String.valueOf(fields.get('LastName'))) + '</li>'
                        + '<li><b>Company:</b> ' + escapeHtml(String.valueOf(fields.get('Company'))) + '</li></ul>';
            return successHtml('Lead Created', body, l.Id, 'View Lead');
        } catch (Exception ex) {
            return errorHtml('Could not create lead', ex.getMessage());
        }
    }
```
...
<!-- Skill-specific: `create_lead` follows the CREATE branch above (substitute object name). -->

## JSON Prompt Command

Lead field API names are passed as **flat top-level keys** (NOT nested under a `fields` object). `LastName` and `Company` are required; common Lead fields are declared explicitly to steer the LLM, and any other standard or custom field (including `__c` fields) can be passed via `additionalProperties: true` and will be picked up by the Apex.
...
```json
{
  ...
  "properties": {
    "LastName": {
      "type": "string",
      "description": "Required. The Lead's last name."
    },
    "Company": {
      "type": "string",
      "description": "Required. The Lead's company name."
    },
    "FirstName": {
      "type": "string",
      "description": "Optional. The Lead's first name."
    },
    "Email": {
      "type": "string",
      "description": "Optional. The Lead's email address."
    },
    "Phone": {
      "type": "string",
      "description": "Optional. The Lead's phone number."
    },
    "Title": {
      "type": "string",
      "description": "Optional. The Lead's job title."
    },
    "Status": {
      "type": "string",
      "description": "Optional. Lead Status picklist API value (call fetch_picklist_values first if unsure)."
    },
    "LeadSource": {
      "type": "string",
    ...
    "Industry": {
      "type": "string",
      "description": "Optional. Industry picklist API value."
    },
    "AnnualRevenue": {
      "type": "number",
