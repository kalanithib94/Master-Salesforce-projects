# Skill: `update_opportunity_fields`
...
when 'update_opportunity_fields'    { return handleUpdateOpportunityFields(parameters); }
    ...
    /**
     * @description Skill 22 — update_opportunity_fields (bulk multi-field,
     *              dependent picklist supported). Field updates are passed as
     *              flat top-level keys (e.g. { opportunity_id, StageName, Amount }).
     *              The legacy nested { "fields": { … } } shape is still accepted
    ...
    private String handleUpdateOpportunityFields(Map<String, Object> p) {
        ...
        if (String.isBlank(oid)) return errorHtml('Could not update opportunity', 'Missing parameter: opportunity_id');
        Map<String, Object> fields = buildFieldMap(p, new Set<String>{ 'opportunity_id', 'recordId', 'Id' });
        if (fields.isEmpty()) return errorHtml('Could not update opportunity', 'Missing parameter: at least one field to update');
        if (!Schema.sObjectType.Opportunity.isUpdateable()) return errorHtml('Could not update opportunity', 'Opportunity is not updateable.');
        List<Opportunity> rows = [SELECT Id, Name FROM Opportunity WHERE Id = :oid WITH USER_MODE LIMIT 1];
        if (rows.isEmpty()) return errorHtml('Could not update opportunity', 'No opportunity found for provided Id.');
        return performGenericUpdate('Opportunity', oid, fields, 'Opportunity Updated', 'View Opportunity', rows[0].Name);
    }
```
...
<!-- Skill-specific: `update_opportunity_fields` — use the matching *_id per Rule 2. -->

## JSON Prompt Command

Field updates are passed as **flat top-level keys** (NOT nested under a `fields` object). Common Opportunity fields are declared explicitly to steer the LLM, and any other standard or custom field (including `__c` fields) can be passed via `additionalProperties: true` and will be picked up by the Apex.
...
```json
{
  ...
  "properties": {
    ...
    "Name": {
      "type": "string",
      "description": "Optional. New Opportunity Name."
    },
    "StageName": {
      "type": "string",
      "description": "Optional. New Stage picklist API value (call fetch_picklist_values first if unsure)."
    },
    "CloseDate": {
      "type": "string",
      "description": "Optional. New close date in yyyy-MM-dd format."
    },
    "Amount": {
      "type": "number",
      "description": "Optional. New amount as a plain number, no currency symbol or commas."
    },
    "Probability": {
      "type": "number",
      "description": "Optional. New probability percentage (0-100) as a plain number."
    },
    "ForecastCategoryName": {
      "type": "string",
      "description": "Optional. New ForecastCategoryName picklist API value."
    },
    "NextStep": {
      "type": "string",
      "description": "Optional. New next-step text."
    },
    "Description": {
      "type": "string",
      "description": "Optional. New long-text description."
    }
  },
