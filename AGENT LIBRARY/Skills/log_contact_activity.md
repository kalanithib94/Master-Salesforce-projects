...
```apex
when 'log_contact_activity'         { return handleLogContactActivity(parameters); }

    private String handleLogContactActivity(Map<String, Object> p) {
        String cid = toText(firstNonNull(p, new List<String>{ 'contact_id', 'recordId', 'Id', 'WhoId' }));
        String subject = toText(p.get('activity_subject'));
        String descr   = toText(p.get('activity_description'));
        if (String.isBlank(cid))     return errorHtml('Could not log activity', 'Missing the contact Id.');
        ...
        try {
            Task t = new Task(WhoId = cid, Subject = subject, Description = descr, Status = 'Completed', Priority = 'Normal', ActivityDate = Date.today());
            insert t;
