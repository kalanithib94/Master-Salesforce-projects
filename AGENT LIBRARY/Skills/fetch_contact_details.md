...
when 'fetch_contact_details'        { return handleFetchContactDetails(parameters); }
    ...
    private String handleFetchContactDetails(Map<String, Object> p) {
        ...
        if (String.isBlank(cid)) return errorJson('Missing required parameter: contact_id');
        ...
        List<Contact> rows = [
            ...
            FROM Contact WHERE Id = :cid WITH USER_MODE LIMIT 1
        ];
        if (rows.isEmpty()) return errorJson('No contact found for provided Id.');
        Contact c = rows[0];
        return successJson(new Map<String, Object>{
            ...
            'MailingAddress' => buildAddress(c.MailingStreet, c.MailingCity, c.MailingState, c.MailingPostalCode, c.MailingCountry),
