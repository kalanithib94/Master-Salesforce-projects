...
when 'update_account_fields'        { return handleUpdateAccountFields(parameters); }
    ...
    private String handleUpdateAccountFields(Map<String, Object> p) {
        ...
        if (String.isBlank(accId)) return errorHtml('Could not update account', 'Missing parameter: account_id');

        Map<String, Object> fields = buildFieldMap(p, new Set<String>{ 'account_id', 'recordId', 'Id' });
        if (fields.isEmpty()) return errorHtml('Could not update account', 'Missing parameter: at least one field to update');

        if (!Schema.sObjectType.Account.isUpdateable()) return errorHtml('Could not update account', 'Account is not updateable.');
        List<Account> rows = [SELECT Id, Name FROM Account WHERE Id = :accId WITH USER_MODE LIMIT 1];
        if (rows.isEmpty()) return errorHtml('Could not update account', 'No account found for provided Id.');
        return performGenericUpdate('Account', accId, fields, 'Account Updated', 'View Account', rows[0].Name);
    }
```
...
<!-- Skill-specific: `update_account_fields` — use the matching *_id per Rule 2. -->
