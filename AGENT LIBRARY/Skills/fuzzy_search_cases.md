...
when 'fuzzy_search_cases'           { return handleFuzzySearchCases(parameters); }
    ...
    private String handleFuzzySearchCases(Map<String, Object> p) {
        String term = toText(firstNonNull(p, new List<String>{ 'search_term', 'subject', 'case_number' }));
        ...
        List<SObject> all = fuzzyQueryMulti(
            ...
            new List<String>{ 'Status', 'Priority', 'AccountId' },
            term
        );
        if (all.isEmpty()) return errorJson('No case found matching "' + term + '".');
        ...
        for (SObject r : top) {
            ...
            records.add(new Map<String, Object>{
                ...
                'recordUrl'  => recordUrl(ca.Id),
                'viewRecord' => viewRecordAnchor(ca.Id)
            });
        }
