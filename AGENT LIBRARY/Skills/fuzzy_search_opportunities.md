# Skill: `fuzzy_search_opportunities`
...
when 'fuzzy_search_opportunities'   { return handleFuzzySearchOpportunities(parameters); }
    ...
    private String handleFuzzySearchOpportunities(Map<String, Object> p) {
        String term = toText(firstNonNull(p, new List<String>{ 'search_term', 'opportunity_name', 'name' }));
        ...
        // Id-based fast path
        if (term.trim().startsWith('006') && term.trim().length() >= 15) {
            try {
                Id oid = Id.valueOf(term.trim());
                List<Opportunity> idR = [SELECT Id, Name, StageName, CloseDate, AccountId, OwnerId FROM Opportunity WHERE Id = :oid WITH USER_MODE LIMIT 1];
                if (idR.isEmpty()) return errorJson('No opportunity found for provided Id.');
                Opportunity o = idR[0];
                return successJson(new Map<String, Object>{
                    ...
                    'records' => new List<Map<String, Object>>{
                        new Map<String, Object>{
                            ...
                            'AccountId' => o.AccountId, 'OwnerId' => o.OwnerId,
                            'recordUrl'  => recordUrl(o.Id),
                            'viewRecord' => viewRecordAnchor(o.Id)
                        }
                    }
                });
            } catch (Exception e) {}
        }
        List<SObject> all = fuzzyQuery('Opportunity', 'Name', new List<String>{ 'StageName', 'CloseDate', 'Amount', 'AccountId', 'OwnerId' }, term);
        if (all.isEmpty()) return errorJson('No opportunity found matching "' + term + '".');
        ...
        for (SObject r : top) {
            ...
            records.add(new Map<String, Object>{
                ...
                'CloseDate' => String.valueOf(r.get('CloseDate')),
                'currentdate__c' => String.valueOf(Date.today()),
                'Amount' => r.get('Amount'),
