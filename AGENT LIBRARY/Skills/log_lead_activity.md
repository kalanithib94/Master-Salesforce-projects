...
```apex
when 'log_lead_activity'            { return handleLogLeadActivity(parameters); }

    private String handleLogLeadActivity(Map<String, Object> p) {
        String lid = toText(firstNonNull(p, new List<String>{ 'lead_id', 'recordId', 'Id', 'WhoId' }));
        String subject = toText(p.get('activity_subject'));
        String descr   = toText(p.get('activity_description'));
        if (String.isBlank(lid))     return errorHtml('Could not log activity', 'Missing the lead Id.');
        ...
        try {
            Task t = new Task(WhoId = lid, Subject = subject, Description = descr, Status = 'Completed', Priority = 'Normal', ActivityDate = Date.today());
            insert t;
            return successHtml('Activity Logged: ' + subject, '<ul><li><b>Status:</b> Completed</li></ul>', t.Id, 'View Task');
        } catch (Exception ex) {
