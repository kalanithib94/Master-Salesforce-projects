...
when 'update_lead_fields'           { return handleUpdateLeadFields(parameters); }
    ...
    /**
     * @description Skill 16 — update_lead_fields. Field updates are passed as
    ...
    private String handleUpdateLeadFields(Map<String, Object> p) {
        ...
        if (String.isBlank(lid)) return errorHtml('Could not update lead', 'Missing parameter: lead_id');
        Map<String, Object> fields = buildFieldMap(p, new Set<String>{ 'lead_id', 'recordId', 'Id' });
        if (fields.isEmpty()) return errorHtml('Could not update lead', 'Missing parameter: at least one field to update');
        if (!Schema.sObjectType.Lead.isUpdateable()) return errorHtml('Could not update lead', 'Lead is not updateable.');
        List<Lead> rows = [SELECT Id, Name FROM Lead WHERE Id = :lid WITH USER_MODE LIMIT 1];
        if (rows.isEmpty()) return errorHtml('Could not update lead', 'No lead found for provided Id.');
        return performGenericUpdate('Lead', lid, fields, 'Lead Updated', 'View Lead', rows[0].Name);
    }
```
...
<!-- Skill-specific: `update_lead_fields` — use the matching *_id per Rule 2. -->

## JSON Prompt Command

Field updates are passed as **flat top-level keys** (NOT nested under a `fields` object). Common Lead fields are declared explicitly to steer the LLM, and any other standard or custom field (including `__c` fields) can be passed via `additionalProperties: true` and will be picked up by the Apex.
...
```json
{
  ...
  "properties": {
    ...
    "Company": {
      "type": "string",
      "description": "Optional. New company name."
    },
    "Email": {
      "type": "string",
    ...
    "Status": {
      "type": "string",
      "description": "Optional. New Lead Status picklist API value."
    },
    "LeadSource": {
      "type": "string",
      "description": "Optional. New Lead Source picklist API value."
    },
    "Industry": {
      "type": "string",
      "description": "Optional. New Industry picklist API value."
    }
  },
